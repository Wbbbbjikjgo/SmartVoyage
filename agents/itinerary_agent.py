"""Itinerary Agent - handles itinerary lifecycle management.

职责边界：仅负责行程/预订的持久化生命周期（创建、查询、更新、取消），
不在此处编排「多 Agent 协作的旅行规划」——真正的规划编排统一收敛到
``orchestrator.workflows.TravelPlanningWorkflow``，避免两处重复实现。
"""
...
class ItineraryAgent(BaseAgent):
    """Agent for itinerary lifecycle management."""
